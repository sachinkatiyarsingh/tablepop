@extends('layouts.app',['class'=>'customers','activePage'=>'planner'])
@section('content')
    ...
    <div class="container">
        <div class="inner-content">
            <div class="left_sidebar">
                ...
                <ul> 
                    <form  method="post" action="{{ url('planners') }}">
                        @csrf
                ...
                <div class="vendor_list">
                    ...
                    <div class="vendor_box move">
                        <div class="vendor_thumb">
                            ...
                            @if(Session::get('type') == 1)
                            <a href="{{ url('planner-profile') }}/{{ $row['id'] ?? '' }}" class="btn-link">View Planner</a>
                            @endif
                            @if(in_array('profile',$planner ??  array()))
                            <a href="{{ url('planner-profile') }}/{{ $row['id'] ?? '' }}" class="btn-link">View Planner</a>
                            @endif
                        ...
                        <div class="vendor_bottom">
                         ...
                         @if(in_array('delete',$planner ??  array()))
                            @if($row['status'] == 0 )
                            <a href="javascript:void(0);" data-id="{{ $row['id'] }}" data-taxt="Active"  data-status="approved" class="btn approved approv" >Active  <span> </span> </a>
                            <a href="javascript:void(0);"  data-id="{{ $row['id'] }}" data-taxt="In-Active"  data-status="decline" class="btn approved decline">In-Active <span> </span> </a>
                      ...
                      @endif
                            <div class="controls">
                                <ul>
                                    ...
                                    @if(Session::get('type') == 1)
                                    <li class="active"><a href="{{ url('planners-edit') }}/{{ $row['id'] ?? '' }}"><img src="{{ asset('resources/assets') }}/demo/images/edit.png" alt=""/></a></li>
